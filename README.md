...
Why AI:
  AI helps increase the chances of users getting a job by ensuring important keywords are included which user might miss while creating multiple resumes. Many times
  users may not know which words would be needed for ATS 90%, but here models learning can help. Such an assistant can help give deserving candidates a chance to better showcase their skills, candidates who are really good at what they do but may not be able to articulate it in the best way for ATS matches.
  
North Star Metrics
  Number of resumes converted to interview process
  Initially let the north star metrics be 80% - an iterative development and eval cycle should help improve this and achieve the goal of 95%
  
Success Criteria
  1. Number of resumes converted to interview process > 95%
  2. Hallucinations < 5%
  3. Changes done to final resume by user < 5%
  4. Users seeing keyword match improvement
  
Counter metrics
  1. Hallucinations < 5% (incorrect company or resume points)
  2. Low Bias
  3. Verbose resume - resumes should be succinct
  4. Tone – is it formal
...
2. RAG
3. Fine Tuning
Since this is an output based on input triggers a basic LLM can be used as MVP. There is not company specific data required here so no need of RAG. LLM with prompting is the
MVP.
AI Apporach for solution:
1. Prompt engineering used to get desired outcome from AI
2. Tweaking the temperature can help provide more creative resumes. I have kept the temperature at 30% as I wanted something more deterministic output than creative as this is based on facts.

UI flow (MVP)
User input -> Job + Resume -> prompts -> LLM -> output resume
...
2. Model can incorporate feedback - adds a feedback look for prompt fine tuning
3. Final resume in PDF based on user feedback
V3:
1. Ask user if they want cover letter
2. Support multi-role resume tweaking
